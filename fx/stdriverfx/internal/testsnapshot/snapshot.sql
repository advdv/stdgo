...
-- ADDED BY HAND: make sure the worker
GRANT USAGE ON SCHEMA "workers" TO "postgres";

-- ADDED BY HAND: to make sure that new tables created via migrations can be used by our postgres.
ALTER DEFAULT PRIVILEGES IN SCHEMA "workers" GRANT
SELECT
, INSERT, UPDATE, DELETE ON TABLES TO "postgres";

-- ADDED BY HAND: to make sure that all sequences can be used
ALTER DEFAULT PRIVILEGES IN SCHEMA "workers" GRANT
SELECT
, USAGE ON SEQUENCES TO "postgres";

-- River main migration 001 [up]
CREATE TABLE workers.river_migration(
...
CREATE UNIQUE INDEX ON workers.river_migration USING btree(version);

-- River main migration 002 [up]
CREATE TYPE workers.river_job_state AS ENUM(
    'available',
    'cancelled',
    'completed',
    'discarded',
...
CREATE TABLE workers.river_job(
    -- 8 bytes
    id bigserial PRIMARY KEY,
    ...
    state workers.river_job_state NOT NULL DEFAULT 'available',
...
CREATE INDEX river_job_kind ON workers.river_job USING btree(kind);

CREATE INDEX river_job_state_and_finalized_at_index ON workers.river_job USING btree(state, finalized_at)
WHERE
    finalized_at IS NOT NULL;

CREATE INDEX river_job_prioritized_fetching_index ON workers.river_job USING btree(state, queue, priority, scheduled_at, id);

CREATE INDEX river_job_args_index ON workers.river_job USING GIN(args);

CREATE INDEX river_job_metadata_index ON workers.river_job USING GIN(metadata);

CREATE OR REPLACE FUNCTION workers.river_job_notify()
    RETURNS TRIGGER
    AS $$
DECLARE
    payload json;
...
CREATE TRIGGER river_notify
    AFTER INSERT ON workers.river_job
    FOR EACH ROW
    EXECUTE PROCEDURE workers.river_job_notify();

CREATE UNLOGGED TABLE workers.river_leader(
    -- 8 bytes each (no alignment needed)
...
ALTER TYPE workers.river_job_state
    ADD VALUE IF NOT EXISTS 'pending' AFTER 'discarded';

ALTER TABLE workers.river_job
    DROP CONSTRAINT finalized_or_finalized_at_null;

ALTER TABLE workers.river_job
    ADD CONSTRAINT finalized_or_finalized_at_null CHECK ((finalized_at IS NULL AND state NOT IN ('cancelled', 'completed', 'discarded')) OR (finalized_at IS NOT NULL AND state IN ('cancelled', 'completed', 'discarded')));

DROP TRIGGER river_notify ON workers.river_job;

DROP FUNCTION workers.river_job_notify;

CREATE TABLE workers.river_queue(
...
ALTER TABLE workers.river_leader
...
BEGIN
    ...
    IF(
        SELECT
            to_regclass('workers.river_migration') IS NOT NULL) THEN
        ALTER TABLE workers.river_migration RENAME TO river_migration_old;
        CREATE TABLE workers.river_migration(
            line text NOT NULL,
            version bigint NOT NULL,
            created_at timestamptz NOT NULL DEFAULT NOW( ),
    ...
    INSERT INTO workers.river_migration(created_at, line, version)
    SELECT
        created_at,
        'main',
        version
    FROM
        workers.river_migration_old;
    DROP TABLE workers.river_migration_old;
END IF;
END;
$body$
LANGUAGE 'plpgsql';
...
CREATE UNIQUE INDEX IF NOT EXISTS river_job_kind_unique_key_idx ON workers.river_job(kind, unique_key)
WHERE
...
--
CREATE UNLOGGED TABLE workers.river_client(
...
-- active client.
CREATE UNLOGGED TABLE workers.river_client_queue(
    river_client_id text NOT NULL REFERENCES workers.river_client(id) ON DELETE CASCADE,
    name text NOT NULL,
    created_at timestamptz NOT NULL DEFAULT now(),
...
CREATE OR REPLACE FUNCTION workers.river_job_state_in_bitmask(bitmask bit(8), state workers.river_job_state)
    RETURNS boolean
    LANGUAGE SQL
    IMMUTABLE
    AS $$
...
-- of non-trivial size to build the index `CONCURRENTLY` out of band of this
-- migration, then follow by completing the migration.
CREATE UNIQUE INDEX IF NOT EXISTS river_job_unique_idx ON workers.river_job(unique_key)
WHERE
    unique_key IS NOT NULL AND unique_states IS NOT NULL AND workers.river_job_state_in_bitmask(unique_states, state);
...
-- unique_states).
DROP INDEX workers.river_job_kind_unique_key_idx;
